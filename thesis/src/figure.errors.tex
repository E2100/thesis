\begin{figure}[t]
  ...
  \begin{tikzpicture}[shorten >=1pt,->,draw=black, node distance=\layersep]
    ...
    \tikzstyle{blank}=[rectangle,right,fill=white,minimum size=20pt,inner sep=5pt]
    ...
    \node[blank] (ER) at (\layersep*2.1,-1) {predicted error};
    \node[blank] (RR) at (\layersep*2.1,0) {predicted rating};
 
    \node[blank] (AG) at (\layersep*3.4,-0.5) {adaption};
    \node[blank] (W) at (\layersep*4.3,-0.5) {$\hat{pr}$}; 
  ...
  \caption[Multiple Models for Adaptive Weights]{
    ...
    Each of the recommenders contribute parts to the final rating.
  }
