...
Let us take a closer look at \emph{baseline ratings}, \emph{dimensionality reduction}, \emph{neighborhood estimation} 
and \emph{social network traversal}. This is by no means an exhaustive list, but rather
...
(3) \emph{Neighborhood estimation} is part of many recommendation systems. 
It is the core principle behind most collaborative filtering algorithms, that
estimate an unknown rating by averaging the ratings of similar users, weighted by this similarity.
...
\begin{equation*}
  pcc(u,v) = \frac{cov(R_u, R_v)}{\sigma_{R_u} \sigma_{R_v}}
\end{equation*}
...
\begin{equation*}
  pcc_{n}(u,v) = 
    \frac{
      \sum_{i=1}^{n} (R_{ui} - \bar{R_u}) (R_{vi} - \bar{R_v}) }{ 
        \sqrt{ \sum_{i=1}^{n} (R_{ui} - \bar{R_u})^2 } 
        \sqrt{ \sum_{i=1}^{n} (R_{vi} - \bar{R_v})^2 }}.
\end{equation*}
...
When a neighborhood is determined, it is time to predict our desired rating.
Basically, this means averaging the neighborhood ratings weighted by similarity:

\begin{equation*}
  \bar{r}_{ui} = \frac{ \sum_{v \in K(u,i)} sim(u,v) \times R_{vi} }{ \sum_{v \in K(u,i)} sim(u,v) },
\end{equation*}

where $K(u,i)$ is the set of users in the neighborhood of $u$ that have rated item $i$.
Naturally, this is the simplest possible way of computing a neighborhood-based prediction.
...
There are many other methods than the PCC used to compute neighborhoods. 
Other simple measures include the \emph{euclidean distance} or \emph{cosine correlation}
between user rating vectors.
\cite{Bell2007a} shows a more sophisticated approach by computing global interpolation weights 
as an optimization problem.
Combinations are also possible. 
\cite{Ujjin} first use a simple metric to gather a larger neighborhood, which is then refined
by a \emph{genetic algorithm}.


(4) \emph{Social networks} traversal, transitive trust
