...
\emph{Baseline ratings} are the simplest family of recommender systems, based on item and user rating averages.
...
highly critical in their assesments, and so on. \citeauthor{Koren2008} compute these baselines by solving the
...
Another approach based on simple averages is the  Slope One family of collaborative filtering algorithms. 
...
that is, we know $r_{uj}$, the predicted rating of $i$ is simply $r_{uj} + \delta$.
...
\emph{Neighborhoods} knn, pearson

\emph{IR methods} vector space, page rank
