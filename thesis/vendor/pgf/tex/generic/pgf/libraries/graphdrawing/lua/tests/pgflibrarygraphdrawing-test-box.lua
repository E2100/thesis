...
--- @release $Header: /cvsroot-fuse/pgf/pgf/generic/pgf/libraries/graphdrawing/lua/tests/pgflibrarygraphdrawing-test-box.lua,v 1.2 2011/04/20 17:50:27 matthiasschulz Exp $
...
function testNewBox()
	local box = Box:new{}
	assert(box.pos.x == 0 and box.pos.y == 0 and box.width == 0
		and box.height == 0)
...
function testAddBox()
	...
	box:addBox(boxnew)
	assert(not boxnew.pos:isAbsPosition())
	assert(boxnew.pos._relativeTo == box.pos)
...
function testRemoveBox()
	...
	assert(countKeys(box._boxes) == 0)
	assert(boxnew.pos:isAbsPosition())
end
...
function testGetPosAtBox()
	local box = Box:new{}
	box.pos.x, box.pos.y = 10, 10
	local boxnew = Box:new{width=10, height=5}
	boxnew.pos.x, boxnew.pos.y = 1, 1
	box:addBox(boxnew)
	local ul, ulabs = boxnew:getPosAt(Box.UPPERLEFT),
		boxnew:getPosAt(Box.UPPERLEFT, true)
