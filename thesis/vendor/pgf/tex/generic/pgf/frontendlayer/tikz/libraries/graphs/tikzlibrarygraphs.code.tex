...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot-fuse/pgf/pgf/generic/pgf/frontendlayer/tikz/libraries/graphs/tikzlibrarygraphs.code.tex,v 1.18 2011/04/21 23:14:35 jannis-pohlmann Exp $
...
\def\tikz@lib@graph@grid@pos{%
  ...
  \tikzgraphpreparewrapafter%
  %
  % find out the coordinate in the grid (at the moment only
  % left to right placement is supported)
  %
  ...
  %
  % multiply by the group shift (which is set to (1,-1) in 
  % the left to right placement strategy)
  %
  \pgfkeysgetvalue{/tikz/graphs/group shift}\tikz@temp%
  \expandafter\tikz@scan@one@point\expandafter\pgf@process\tikz@temp%
  \pgfmathsetlength{\pgf@x}{\pgf@x * \tikz@temp@col}
  \pgfmathsetlength{\pgf@y}{\pgf@y * \tikz@temp@row}
  %
