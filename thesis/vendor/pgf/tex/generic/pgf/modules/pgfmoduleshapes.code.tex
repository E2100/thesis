...
\ProvidesFileRCS $Header: /cvsroot-fuse/pgf/pgf/generic/pgf/modules/pgfmoduleshapes.code.tex,v 1.16 2011/04/13 19:56:44 matthiasschulz Exp $
...
\def\pgfmultipartnode#1#2#3#4{%
  ...
  {%
    {%
      ...
      \else%
        % Rename node
        \edef\pgf@node@name{not yet positionedPGFGDINTERNAL\pgf@node@name}%
...
\def\pgfpositionnodenow@rename not yet positionedPGFGDINTERNAL#1\relax{%
  \pgfnoderename{#1}{not yet positionedPGFGDINTERNAL#1}
  \pgf@nodecallback{#1}%
}
